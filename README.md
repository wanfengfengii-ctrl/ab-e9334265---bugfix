...
共 2m 个配置。求解分两阶段，所有配置的 DP 均以 numpy 向量化批量推进（按参考
前缀逐行滚动）：

1. **代价 DP**：`dp[i][j]` 为参考环前 i 个间隔与实测环前 j 个间隔对齐的最优
   字典序代价，转移枚举最后一组两侧跨度 (a, b) ∈ {1,2,3}²，逐格贪心即得全局
   最优目标 B = (改动数, 总绝对误差, 最大组误差)；
2. **计数 DP**：全局最优路径的前缀在三元比较下未必局部最优（可能仅最大组误差
   分量更大、靠后续大误差组拉平），故以最优最大组误差为组误差上限，对可加的
   二元代价 (改动数, 总绝对误差) 重做 DP 并汇总并列最优转移，得到最优路径的
   **精确计数**（int64 快速推进并在高位阈值处饱和探测，一旦触及即以 Python
   任意精度整数重算，因此计数可超出 int64 / JS 安全整数而保持精确）。
