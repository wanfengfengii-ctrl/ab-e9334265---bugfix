...
共 2m 个配置，分两遍 numpy 向量化 DP（按参考前缀逐行滚动，每行对所有配置与
实测位置同时推进）：

1. **代价遍**：`dp[i][j]` 为参考环前 i 个间隔与实测环前 j 个间隔对齐的最优
   字典序代价 (改动数, 总绝对误差, 最大组误差)，转移枚举最后一组两侧跨度
   (a, b) ∈ {1,2,3}²。最大组误差是**非加性瓶颈量**——同前缀下当前最大误差
   较大的方案可能被后续同量级误差追平，故此遍只用于求最优目标值，不用于计数；
2. **计数遍**：以最优最大组误差为硬约束（每组误差 ≤ e*），对加性目标
   (改动数, 总绝对误差) 再做一遍 DP 并对并列转移求和。目标变成纯加性后无剪枝
   漏计，达到终点最优 (m*, t*) 的路径恰为全部最优规范映射。

计数以 int64 快速推进并在高位阈值处饱和探测，一旦触及即以 Python 任意精度
整数重算，因此计数可超出 int64 / JS 安全整数而保持精确。见证重构使用同一
“硬 cap + 加性目标”的标量 DP 回溯，可恢复被代价遍暂时支配的并列切分。
